...
`max_results`, `…` that it is still running). `<C-o>` and `<C-x>` light up
that strip and mark each entry with its key rather than opening anything.
...
| `LoupeTabSelect` / `LoupeTabSelectActive` | `Visual` / `PmenuSel` | the strip while choosing a source |
